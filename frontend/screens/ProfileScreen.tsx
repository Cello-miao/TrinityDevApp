...
export default function ProfileScreen({ navigation }: any) {
  ...
  const [scanInfoAnchor, setScanInfoAnchor] = useState({
    x: 0,
    y: 0,
    width: 0,
    height: 0,
  ...
  const loadScannerPreference = async () => {
    try {
      const storedValue = await AsyncStorage.getItem(
        USER_SCAN_AUTO_OPEN_DETAIL_KEY,
      );
      setUserScanAutoOpenDetail(
  ...
  const handleFontScaleStep = (delta: number) => {
    const nextScale = Math.min(
      1.4,
      Math.max(0.8, Number((fontScale + delta).toFixed(2))),
    );
  ...
  const estimatedPopoverHeight =
    scanInfoPopoverHeight || Math.round(128 * fontScale);
  ...
  const popoverTopWhenBelow =
    scanInfoAnchor.y + scanInfoAnchor.height + anchorGap;
  const popoverTopWhenAbove =
    scanInfoAnchor.y - estimatedPopoverHeight - anchorGap;
  const canOpenBelow =
    popoverTopWhenBelow + estimatedPopoverHeight <=
    windowHeight - bottomSafeInset;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.profileSummaryCard}>
          ...
          <View style={styles.profileSummaryContent}>
            ...
            <Text style={styles.profileSummaryEmail}>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setShowScanInfo(false)}
      >
        <Pressable
          style={styles.popoverBackdrop}
          onPress={() => setShowScanInfo(false)}
        >
          <Pressable
            style={[
              styles.popoverCard,
            ...
            onLayout={(event) =>
              setScanInfoPopoverHeight(event.nativeEvent.layout.height)
            }
          ...
          >
            ...
            <Text style={styles.popoverText}>
              Enabled: after a successful scan, the app opens product details
              immediately.
            </Text>
            <Text style={styles.popoverText}>
              Disabled: after scan, you can choose to view details or continue
              scanning.
            </Text>
...
const createStyles = (theme: any, getFontSize: (baseSize: number) => number) =>
  StyleSheet.create({
    container: {
      flex: 1,
